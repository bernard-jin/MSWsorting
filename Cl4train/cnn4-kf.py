...
def tfone_hot(t_tf):
    t = t_tf.long()
    t = F.one_hot(t, num_classes=2).to(torch.float32)
    return t

for tol in [0.04]:
    core_path = os.path.join(path, 'pkls/hefei/Cl/cnn3')
    viz.close()
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(combined_set)):
        ...
        trainer = DataLoader(train_dataset, shuffle=True, batch_size=256)
        tester = DataLoader(test_dataset, shuffle=True, batch_size=256)
        ...
        network = MLCP(sizes=headsize, drops=dropsize, out=out)
        network.to(device)
            ...
            for idx, [x, label] in enumerate(trainer):
                ...
                if epoch >= int(0.9*epochs):
                    if vacc_max < acc_valid_class:
                        ...
                        torch.save(network, os.path.join(core_path, 'tol{}_F{}ep{}idx{}_T{}V{}.pkl'.format(tol, fold, epoch, idx, round(acc_class,4), round(vacc_max,4))))
                        print('saving..')
                    else:
                        continue
    ...
    with open(os.path.join(core_path, output_filename), 'w') as output_file:
        with contextlib.redirect_stdout(output_file):
            ...
            params['tol'] = tol
            params['positive'] = sum(a >= tol * label_rate for a in set1.labels) + sum(a >= tol * label_rate for a in set2.labels)
            params['negative'] = sum(a < tol * label_rate for a in set1.labels) + sum(a < tol * label_rate for a in set2.labels)
            print(kf)
            print(params)
            print(network)
            print(criterion)  # mse
