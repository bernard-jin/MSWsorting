...
import br3train.losses as losses
...
# 网络结构的参数配置
headsize = [1, 256, 256, 84]
headout = 10000
tailsize, tol = [headout, 2], 2
...
for fold, (train_indices, val_indices) in enumerate(kf.split(combined_set)):
    ...
    trainer = DataLoader(Subset(combined_set, train_indices), batch_size=batchsize, shuffle=True, num_workers=8)
    tester = DataLoader(Subset(combined_set, val_indices), batch_size=valsize, shuffle=True, num_workers=8)
