...
# core_path = '/home/jzq/PycharmProjects/Br/Question-塑料分析/pkls/mlp3'
# files = os.path.join(path, 'Data3/br')
# files = os.path.join(path, 'Data3/br4code')
# files = os.path.join(path, 'Data3/br4kernel')

# if hefei
core_path = os.path.join(path, 'pkls/hefei/mlp3')
...
for tol in [250]:
            ...
            for idx, [x, label] in enumerate(trainer):
                ...
                with (torch.no_grad()):
                    ...
                    for x_valid, label_valid in tester:
                        ...
                        vconfu += confusion_matrix(label_valid_class.cpu(), y_valid_class.cpu(), labels=[False, True])   # y_TF confuse with label_TF
                ...
                if (epoch >= int(0.9*epochs)) & (files.split('/')[-1] != 'br4code'):
                    if vacc_max < acc_valid_class:
                        ...
                        # torch.save(network, os.path.join(core_path, 'mlp_tol{}_F{}ep{}idx{}_T{}V{}.pkl'.format(tol/1000, fold, epoch, idx, round(acc_class,4), round(vacc_max,4))))
                        # print('saving..')
                    else:
                        continue
    ...
    with open(os.path.join(core_path, output_filename), 'w') as output_file:
        with contextlib.redirect_stdout(output_file):
            ...
            params['tol'] = tol
            params['positive'] = sum(a >= tol for a in set.labels)
            params['negative'] = sum(a < tol for a in set.labels)
            print(kf)
            print(params)
            print(network)
            print(criterion)  # mse
