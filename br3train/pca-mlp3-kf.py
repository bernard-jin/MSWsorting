...
# if ubuntu, ubuntu path
path = '/home/jzq/PycharmProjects/Br/Question-塑料分析'
# core_path = os.path.join(path, 'pkls/hefei/pca-mlp3')
br = os.path.join(path, 'BR.xlsx')
...
# get a pca module
files = os.path.join(path, 'Data3/HeFei/xlsx/NB_notcol_avg')
kset = Brset(specroot=files, labelroot=br, mode='')
...
if not os.path.exists(pca_path):
    # pca 塑性
    pca = KernelPCA(n_components=n_compo, kernel=kernel)
    pca.fit(kset.specs)
    # save the pca module
    if files.split('/')[-1] != 'br4code':
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(set)):
        ...
        for epoch in range(epochs):
            for idx, [x, label] in enumerate(trainer):
                ...
                # 1 inputting
                x = torch.from_numpy(pca.transform(x))
                ...
                # loss = criterion(y, label)  # 3.1， if y is ,1)
                loss = criterion(y, tfone_hot(label_class))  # 3.2， if y is ,2)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                if (epoch >= int(0.9 * epochs)) & (files.split('/')[-1] != 'br4code'):
                    if vacc_max < acc_valid_class:
                        ...
                        torch.save(network, os.path.join(core_path, 'tol{}_F{}ep{}idx{}_T{}V{}.pkl'.format(tol/1000, fold, epoch, idx, round(acc_class,4), round(vacc_max,4))))
                        print('saving..')
                    else:
                        continue
