...
from modelclasses.nets import AE, loss_function
...
core_path = os.path.join(path, 'pkls/hefei/coders')
...
# build vae train object
# model = VAE(195, 80, 60).to(device)
model = AE(195, 255, 80).to(device)
...
for epoch in range(epochs):
    for batchidx, [x, label] in enumerate(spectrum_loader):
        ...
        # loss = loss_function(x_hat, x, miu, logvar)
        loss = criterion(x_hat*100, x*100)
